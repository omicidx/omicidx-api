...
def encode_cursor(sort_dict: list, resp) -> bytes:
    sort_items = []
    for d in sort_dict:
        sort_items.append("{}||{}".format(*d.popitem()))
    cursor_string = "|||".join(sort_items) + "||||{}".format(resp)
    return base64.encodebytes(cursor_string.encode('UTF-8'))



def decode_cursor(cursor_string: bytes):
    (kvs, id) = base64.decodebytes(cursor_string).split(b"||||")
