...
#            continue
        if(details['type']=='text'):
            ...
            short_fields[name] = graphene.List(AttributeType, q=graphene.String())
...
# not used right now.
typemap = {'SRAStudyType':'sra_study',
...
class M(graphene.InputObjectType):
    def __init__(self, fieldnames):
        self.enumfield = graphene.InputField(
            graphene.Enum('Fields', list([(fieldname, fieldname)
...
exptfield = graphene.Field(SRAExperimentType,description = "Fetch a single entity by its accession",
                           accession = graphene.String(description = 'Lookup by a single accession'))



enumtest = graphene.Enum('enumtest', (('study','study'),('study_accession',['study.accession','abc'])))
...
def gen_get_by_accession(cls, index):
    ...
    return graphene.Field(
        cls,
        accession = graphene.String(
            description = 'Lookup by a single accession'),
        resolver = resolver
    )
